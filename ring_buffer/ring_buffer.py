...
class RingBuffer:
    ...
    def append(self, item):
        ...
        if self.storage.length < self.capacity:
            ...
            # now our lil variable has been added and is z
            self.z = self.storage.tail
            return
        else:
            # if the lil variable is already there start back over at the beginning of the head
            if self.z == self.storage.tail:
                self.z = self.storage.head
            # otherwise lil variable will be added to the next one
            else:
                self.z = self.z.next
            self.z.value = item
